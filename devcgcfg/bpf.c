...
int bpf_program_append_device(struct bpf_program *prog, char type, int major, int minor, char *access)
{
	...
	ret = bpf_access_mask(access, &access_mask);
	if (ret < 0)
		return log_error_errno(ret, -ret, "Invalid access mask specified %s", access);
	...
	if (device_type < 0)
		return log_error_errno(-1, EINVAL, "Invalid bpf cgroup device type %c", type);
	...
	bpf_access_decision[0] = BPF_MOV64_IMM(BPF_REG_0, 1);
...
int bpf_program_finalize(struct bpf_program *prog)
{
	...
	/*
	printf("Implementing %s bpf device cgroup program",
